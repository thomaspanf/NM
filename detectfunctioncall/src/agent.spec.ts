...
describe("Nethermind bot deploy bot test suite", () => {
    ...
    describe("handleTransaction", () => {
        ...
        it('should ignore other calls from NETHERMIND to FORTA', async () =>{
            const tx: TransactionEvent = new TestTransactionEvent().setFrom(createAddress(NETHERMIND_ADDRESS)).setTo(FORTA_ADDRESS)
            const findings = await agent.handleTransaction(tx);
            expect(findings).toStrictEqual([]); 
        });
